...
def score_pattern(pattern, planetary_data, role_to_planet, role_weights=None, retrograde_multiplier=1.5):
    """
    Score how well planetary data matches the geometry's match_pattern.

    Args:
        pattern (dict): Full archetype pattern containing 'match_pattern'.
    ...
    """
    match_pattern = pattern.get("match_pattern", {})
    score = 0
    mismatches = []
    retrograde_roles = []

    for role, expected in match_pattern.items():
